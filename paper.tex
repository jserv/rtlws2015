...
The Linux kernel running under Xenomai 3 can be PREEMPT\_RT as well through the native option of Xenomai 3 for maximum API/ABI compatibility in real-time environments. For the tight latency requirements such as real-time networking, Xenomai 3 allows to switch to dual kernel configurations utilizing I-pipe for maximum real-time performance, which brings sligtly better performance than Xenomai 2 in general. However, comparing to Xenomai 2, our results show the unexpected slowdown of timer interrupt hanlding latency in stressed condition while Xenomai 3 was configured as the dual kernel configuration of Xenomai 3 though. It is not clear why Cobalt in Xenomai 3 contributes to larger interrupt jitter when the CPU is heavily loaded. This needs further investigation.
