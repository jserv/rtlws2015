...
In Xenomai 3, with the introduction of the \textit{Copperplate} interface, which mediates between the real-time API / emulator of given application uses, and the underlying real-time core. That is, applications are able to run in either environments without visible code change. We do not intend to measure the overhead of \textit{Copperplate} interface or its implementation for the sake of measuring the system-level performance rather than API-oriented emulation.
