...
A Linux kernel that can be patched with PREEMPT\_RT, Xenomai 2 and Xenomai 3 is needed for the experiment (Xenomai 3 has to run on Linux kernel 3.10 or later versions). Linux 3.14 is selected for our experiment. In some cases when the patches couldn't be successfully applied on the kernel, we tried to resolve the conflicts manually.
